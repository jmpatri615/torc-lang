//! End-to-end arithmetic — binary entry point.
//!
//! Usage:
//!   cargo run -p e2e-arithmetic [-- <output-dir>]
//!
//! Default output directory: /tmp/e2e-arithmetic
...
name = "e2e-arithmetic"
version = "0.1.0"
description = "End-to-end arithmetic: (10 + 32) * 2 - 5 = 79"
...
fn main() {
    let out_dir: PathBuf = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "/tmp/e2e-arithmetic".into())
        .into();

    println!("Building arithmetic graph: (10 + 32) * 2 - 5 = 79");
    let graph = e2e_arithmetic::build_graph();
    ...
    println!("  torc verify              # structural + interval verification");
    println!("  torc inspect --view pseudo-code");
    println!("  torc inspect --view contracts");
    println!("  torc inspect --view dataflow");
}
