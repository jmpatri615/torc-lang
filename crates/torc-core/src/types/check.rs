...
pub fn types_compatible(
    source: &Type,
    target: &Type,
) -> Result<Vec<ProofObligation>, TypeError> {
...
fn check_compatible(
    ...
    match (source, target) {
        ...
        (
            Type::Int { width: sw, signedness: ss },
            Type::Int { width: tw, signedness: ts },
        ) => {
            if sw == tw && ss == ts {
                Ok(())
            } else {
                Err(incompatible(source, target, "integer width or signedness mismatch"))
            }
        }

        (
            Type::Float { precision: sp },
            Type::Float { precision: tp },
        ) => {
            if sp == tp {
                Ok(())
            } else {
        ...
        (
            Type::Fixed { total_bits: st, frac_bits: sf },
            Type::Fixed { total_bits: tt, frac_bits: tf },
        ) => {
            if st == tt && sf == tf {
                Ok(())
            } else {
        ...
        (
            Type::Array { element: se, length: sl },
            Type::Array { element: te, length: tl },
        ) => {
            if sl != tl {
        ...
        (Type::Refined { base: sb, predicate: sp }, Type::Refined { base: tb, predicate: tp }) => {
        ...
        (src, Type::Refined { base: tb, predicate: tp }) if !matches!(src, Type::Refined { .. }) => {
        ...
        (
            Type::Linear { inner: si, linearity: sl },
            Type::Linear { inner: ti, linearity: tl },
        ) => {
        ...
        (
            Type::Timed { inner: si, bound: sb },
            Type::Timed { inner: ti, bound: tb },
        ) => {
            check_compatible(si, ti, obligations)?;
        ...
        (
            Type::Sized { inner: si, max_bytes: sb },
            Type::Sized { inner: ti, max_bytes: tb },
        ) => {
            check_compatible(si, ti, obligations)?;
            ...
            if sb > tb {
                return Err(TypeError::ResourceBoundExceeded {
                    kind: "Sized",
                    detail: format!(
                        "source size {}B exceeds target size {}B",
                        sb, tb
                    ),
                });
            }
        ...
        (
            Type::Powered { inner: si, energy_uj: se },
            Type::Powered { inner: ti, energy_uj: te },
        ) => {
            check_compatible(si, ti, obligations)?;
            ...
            if se > te {
                return Err(TypeError::ResourceBoundExceeded {
                    kind: "Powered",
                    detail: format!(
                        "source energy {}μJ exceeds target energy {}μJ",
                        se, te
                    ),
                });
            }
        ...
        (
            Type::Bandwidth { inner: si, min_bps: sb },
            Type::Bandwidth { inner: ti, min_bps: tb },
        ) => {
            check_compatible(si, ti, obligations)?;
            ...
            if sb < tb {
                return Err(TypeError::ResourceBoundExceeded {
                    kind: "Bandwidth",
                    detail: format!(
                        "source throughput {}Bps below target minimum {}Bps",
                        sb, tb
                    ),
                });
            }
        ...
        // === Probability types ===
        (Type::Distribution(si), Type::Distribution(ti)) => {
            check_compatible(si, ti, obligations)
        }

        (
            Type::Posterior { inner: si, evidence: se },
            Type::Posterior { inner: ti, evidence: te },
        ) => {
            if se != te {
        ...
        (
            Type::Interval { inner: si, confidence: sc },
            Type::Interval { inner: ti, confidence: tc },
        ) => {
        ...
        (
            Type::Approximate { inner: si, max_error: se },
            Type::Approximate { inner: ti, max_error: te },
        ) => {
            if (se - te).abs() > f64::EPSILON {
                return Err(incompatible(source, target, "approximate error bound mismatch"));
            }
            check_compatible(si, ti, obligations)
        }
        ...
        // === Dependent types ===
        (
            Type::Parameterized { name: sn, type_params: stp, value_params: svp },
            Type::Parameterized { name: tn, type_params: ttp, value_params: tvp },
        ) => {
            if sn != tn {
                return Err(incompatible(source, target, "parameterized type name mismatch"));
            }
            if stp.len() != ttp.len() {
                return Err(incompatible(source, target, "type parameter count mismatch"));
            }
            if svp.len() != tvp.len() {
                return Err(incompatible(source, target, "value parameter count mismatch"));
            }
...
mod tests {
    ...
    fn linearity_compatible_lattice() {
        ...
        assert!(linearity_compatible(Linearity::Counted, Linearity::Unrestricted));
    }
}
